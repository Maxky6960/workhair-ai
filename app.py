...
def require_env(name: str, value: Optional[str]) -> None:
    if value:
        return
    st.error(f"ยังไม่ได้ตั้งค่า {name} กรุณาตรวจไฟล์ .env หรือ secret ของระบบ")
    st.stop()
...
def friendly_error_message(exc: Exception) -> str:
    ...
    if "quota" in lower_message or "resource_exhausted" in lower_message:
        return "API ถูกใช้งานเกิน quota ชั่วคราว กรุณารอสักครู่แล้วลองใหม่อีกครั้ง"
    if "api key" in lower_message or "apitoken" in lower_message:
        return "ระบบยังไม่ได้ตั้งค่า API key กรุณาตรวจไฟล์ .env หรือ secret ของระบบ"
    if "deadline" in lower_message or "timeout" in lower_message:
        return "ระบบใช้เวลาตอบนานเกินไป กรุณาลองใหม่อีกครั้ง"
    if "connection" in lower_message or "network" in lower_message:
        return "ไม่สามารถเชื่อมต่อบริการ AI ได้ กรุณาตรวจอินเทอร์เน็ตแล้วลองใหม่อีกครั้ง"
    return "ขออภัย ระบบขัดข้องชั่วคราว กรุณาลองใหม่อีกครั้ง"


def safe_generate(system_instruction: str, user_prompt: str, max_retries: int = 3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "quota" in message.lower() or "RESOURCE_EXHAUSTED" in message:
                wait = 2 ** attempt
                logger.warning("gemini_rate_limited wait_seconds=%s", wait)
                time.sleep(wait)
            else:
                raise
...
def call_gemini(system_instruction: str, user_prompt: str) -> str:
    try:
        start = time.time()
        response = safe_generate(system_instruction, user_prompt)
