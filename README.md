...
- มีหน้า admin สำหรับบันทึกยอดขายลง Google Sheets
- มี `sales_logger.py` สำหรับบันทึกยอดขายผ่าน terminal ตามรูปแบบ Session 2
...
├── sales_logger.py           # Sales logger script สำหรับบันทึกยอดขาย
...
สำหรับ Google Sheets ต้องแชร์ Sheet ให้ `client_email` ในไฟล์ service account JSON เป็นสิทธิ์ Editor ก่อนใช้งาน
...
หน้า admin สำหรับบันทึกยอดขาย

```text
http://localhost:8501/?page=admin
```

## Sales Logger Script

บันทึกยอดขายผ่าน terminal ได้ด้วยคำสั่ง

```bash
python sales_logger.py "ตัดผมชาย:1:150:ช่างมิน:โอนผ่านธนาคาร:walk-in"
```

รูปแบบข้อมูลคือ

```text
บริการ:จำนวน:ราคา:ช่าง:วิธีชำระเงิน:หมายเหตุ
```

Google Sheet ควรมีหัวตารางแถวแรกดังนี้

```text
วันที่, บริการ, จำนวน, ยอดรวม, ช่าง, วิธีชำระเงิน, หมายเหตุ
```
...
- [x] มี Sales Logger สำหรับบันทึกยอดขายลง Google Sheets
